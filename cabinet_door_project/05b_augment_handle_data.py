...
New features added per timestep (7 dims total):
  ...
  handle_to_eef_pos   (3)  EEF position relative to handle (eef_pos - handle_pos)
...
def find_fixture_handle_sites(model, fixture_name):
    """Find MuJoCo site names for the target fixture's door handles."""
    sites = []
...
def match_handle_site(handle_body, handle_sites):
    """Pick the best handle site name for a given handle body name."""
    if not handle_sites:
        return None
    hb_lower = handle_body.lower()
    if "left" in hb_lower:
        for s in handle_sites:
            if "left" in s.lower():
                return s
    if "right" in hb_lower:
        for s in handle_sites:
            if "right" in s.lower():
                return s
    for s in handle_sites:
...
def process_episode(ep_dir, model_cache=None):
    ...
    # Find handle bodies / sites and door joints
    handle_bodies = find_fixture_handle_bodies(model, fixture_name)
    handle_sites = find_fixture_handle_sites(model, fixture_name)
    ...
    for t in range(num_steps):
        ...
        handle_site = match_handle_site(target_handle, handle_sites)
        if handle_site is not None:
            handle_pos = data.site(handle_site).xpos.copy()
        else:
